...
function MWeaponTruck:constructor()
	...
	self:createStartPoint(2799.58, -2455.13, 12.73, "state")
		...
		if faction then
			if (faction:isEvilFaction() and source.type == "evil") or (faction:isStateFaction() and source.type == "state" and hitElement:isFactionDuty()) then
				...
				if source.type == "evil" then
					hitElement:sendError(_("Den Waffentruck können nur Mitglieder einer böser Fraktionen starten!",hitElement))
				elseif source.type == "state" then
					hitElement:sendError(_("Den Staats-Waffentruck können nur Staatsfraktionisten im Dienst starten!",hitElement))
				end
			end
		else
		...
		if faction:isEvilFaction() then
			if client:isFactionDuty() then
				if FactionState:getSingleton():countPlayers() < WEAPONTRUCK_MIN_MEMBERS["evil"] and not DEBUG then
					client:sendError(_("Es müssen mindestens 2 Staatsfraktionisten online sein!", client))
					return
				end
		...
		elseif faction:isStateFaction() then
			if client:isFactionDuty() then
				if FactionEvil:getSingleton():countPlayers() < WEAPONTRUCK_MIN_MEMBERS["state"] and not DEBUG then
					client:sendError(_("Es müssen mindestens 2 Spieler böser Fraktionen online sein!", client))
					return
				end
		...
		if faction then
			...
			if faction:getMoney() >= totalAmount then
				if totalAmount > 0 then
					...
					client:sendError(_("Du hast zuwenig aufgeladen! Mindestens: %d$",client,self.m_AmountPerBox))
				end
			else
				client:sendError(_("Ihr habt nicht genug Geld in der Fraktions Kasse! (%d$)",client,totalAmount))
			end
		else
