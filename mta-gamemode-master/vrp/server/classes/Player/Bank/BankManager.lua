...
function BankManager:Event_Transfer(toPlayerName, amount, purpose)
	...
	if tonumber(amount) and amount > 0 then
		...
		elseif toPlayerName == "GRS Event-Team" then
			client:transferBankMoney(Admin:getSingleton().m_BankAccount, amount, ("Spende an GRS Event-Team von %s"):format(client:getName()), "Gameplay", "eXoTeamDonation")
			client:triggerEvent("bankMoneyBalanceRetrieve", client:getBankMoney())
		else
