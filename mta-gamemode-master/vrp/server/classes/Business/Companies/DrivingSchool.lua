...
DrivingSchool.testRoute =
{
	...
	{1833.45, -1754.71, 13.38},
	{1981.06, -1754.93, 13.39},
	{2183.47, -1753.66, 13.37},
	{2230.12, -1734.67, 13.38},
	{2444.64, -1735.37, 13.56},
	{2632.06, -1735.64, 10.77},
	{2657.79, -1659.84, 10.70},
	{2740.51, -1639.68, 12.94},
	{2734.64, -1525.03, 29.53},
	{2705.82, -1504.59, 30.36},
	{2652.91, -1401.81, 30.27},
	{2641.61, -1435.88, 30.27},
	{2404.43, -1442.29, 23.83},
	{2379.91, -1381.92, 23.86},
	{2141.86, -1381.64, 23.82},
	{2109.83, -1400.60, 23.82},
	{2110.28, -1452.29, 23.82},
	{2081.16, -1459.90, 23.68},
	{1867.16, -1459.99, 13.38},
	{1844.86, -1494.24, 13,36},
	{1819.58, -1678.18, 13.38},
...
function DrivingSchool:Event_startAutomaticTest(type)
	...
	if #self:getOnlinePlayers() >= 1 then
		client:sendWarning(_("Es sind genügend Fahrlehrer online!", client))
		return
	end

	local valid = {["car"]= true, ["bike"] = true, ["truck"] = true }
...
function DrivingSchool:startAutomaticTest(player, type)
	...
	local veh  = TemporaryVehicle.create(type == "car" and 410 or type == "bike" and  521, 1761.76, -1687.31, 13.02, 180) or TemporaryVehicle.create(type == "truck" and 578, 1761.76, -1687.31, 15.02, 270)
		...
		function(player, seat)
			if source.m_Driver == player then
				if seat == 0 then
					...
					if player.m_AutoTestMode == "truck" then
						setTimer(outputChatBox, 4000, 1, _("%s sagt: Nun Anschnallen mit 'M'", player, name) , player, 200, 200, 200)
					end
				end
			else
	...
	if DrivingSchool.m_LessonVehicles[client] then
		...
		if veh.m_CurrentNode <= #DrivingSchool.testRoute then
			...
			if getElementHealth(veh) >= 500 then
				...
				if veh.m_TestMode == "bike" then
					client.m_HasBikeLicense = true
				end
				if veh.m_TestMode == "truck" then
...
function DrivingSchool:Event_endLession(target, success, clientServer)
    ...
    if success == true then
		...
		if mileageDiff < 3 then
			client:sendWarning("Du musst mindestens 3km mit dem Fahrschüler fahren!")
			return
		end
