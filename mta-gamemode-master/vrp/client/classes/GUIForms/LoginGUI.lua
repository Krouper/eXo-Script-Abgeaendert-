...
function LoginGUI:loadLoginElements()
	...
	self.m_Elements.lbl1 = GUIGridLabel:new(1, 3, 9, 2, _"Herzlich willkommen auf GRS Reallife, bitte logge dich mit deinen Accountdaten ein.", self.m_Window)
		:setAlignX("center")
...
function LoginGUI:showRegisterMultiaccountError(name)
	...
	local text = _("Für deine Serial existiert bereits ein Account. Wenn du mit anderen Spielern im gleichen Netzwerk spielen möchtest, musst du einen Multiaccount-Antrag im Forum (forum.grs-reallife.de) verfassen.")
	if name then
		text = _("Deine Serial wurde zuletzt vom Spieler '%s' benutzt! Wenn du mit anderen Spielern im gleichen Netzwerk spielen möchtest, musst du einen Multiaccount-Antrag im Forum (forum.grs-reallife.de) verfassen.", name)
	end
