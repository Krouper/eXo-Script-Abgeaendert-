...
function FireManager:updateStatistics(tblStats, timeSinceStart, timeEstimated, w, h)
	if not self.m_StatisticShortMessage[tblStats.name] then
		...
		playSound("files/audio/Piepser.mp3")
	end
...
addEvent("fireAlarm", true)
addEventHandler("fireAlarm", root,
	function(x, y, z)
        -- Play an alarm for 5min
        firesound = Sound3D.create("files/audio/Feuer.mp3", x, y, z, false)
		Sound3D.setMaxDistance(firesound, 60)
    end
)

addEvent("fireAlarmStop", true)
addEventHandler("fireAlarmStop", root,
    function()
        if isElement(firesound) then firesound:destroy() end
    end
)
